...
namespace Al.Security.Pkix
{
    public class PkixNameConstraintValidator
    {
        ...
        private ISet UnionIPRange(byte[] ipWithSubmask1, byte[] ipWithSubmask2)
        {
            ...
            if (Al.Security.Utilities.Arrays.AreEqual(ipWithSubmask1, ipWithSubmask2))
            {
            ...
            byte[] ipWithSubmask2)
    {
        ...
        private bool IsIPConstrained(byte[] ip, byte[] constraint)
        {
            ...
            return Al.Security.Utilities.Arrays.AreEqual(permittedSubnetAddress, ipSubnetAddress);
        }
        ...
        {
            if (Al.Security.Utilities.Arrays.AreEqual(ip1, ip2))
            {
                return 0;
            }
            if (Al.Security.Utilities.Arrays.AreEqual(Max(ip1, ip2), ip1))
            {
                return 1;
            }
            return -1;
        ...
        private int HashCollection(ICollection coll)
        {
            ...
            while (it1.MoveNext())
            {
                ...
                if (o is byte[])
                {
                    hash += Al.Security.Utilities.Arrays.GetHashCode((byte[])o);
                }
                else
                {
                    hash += o.GetHashCode();
        ...
        private bool SpecialEquals(Object o1, Object o2)
        {
            ...
            {
                return Al.Security.Utilities.Arrays.AreEqual((byte[])o1, (byte[])o2);
            }
            else
            {
                return o1.Equals(o2);
